...
#' @rdname data.getBhavcopy
#' @return Returns a dataframe with all the accumulated data.
#' @export
data.getBhavcopy <- function(startDate = as.Date("2010-01-01",order="ymd"),
  ...
  if( class( temp ) == "try-error" || is.na( temp ) ) {
    stop("In data.getBhavcopy : startDate is not valid")
  }
  
  temp <- try( as.Date( endDate, format= "%Y-%m-%d" ) )
  if( class( temp ) == "try-error" || is.na( temp ) ) {
    stop("In data.getBhavcopy : endDate is not valid")
  }
  
  if (startDate > endDate){
    stop("In data.getBhavcopy : startDate is greater than endDate")
  }
  
  if (is.null(dirName)){dirName <- getwd()}
  if (!dir.exists(dirName)){
    stop("In data.getBhavcopy: directory name given in dirName doesn't exist")
  }
