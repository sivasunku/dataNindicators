...
#' @rdname data.getQuotesFromBhavcopy
#' @return Returns a dataframe with all the OHLC data
#' @export
#' 
data.getQuotesFromBhavcopy <- function(
                       stock     = NULL,
                       fileName  = NULL,
                       objectName = NULL) {
  temp <- NULL
  ...
  if (is.null(fileName)  && is.null(objectName)){
    stop("In data.getQuotesFromBhavcopy : both filename and objectname are null")
  }
  if (!is.null(fileName) && (!file.exists(fileName)) ){
    stop("In data.getQuotesFromBhavcopy : filename doesn't exist")
  }
