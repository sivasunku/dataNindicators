...
#' @export
data.makeQuotesFromGDrive <- function(dirName = ".",echoFlag = FALSE){
  
  if (!dir.exists(dirName)){
   stop("In data.makeQuotesFromGDrive : dirName doesnot exist") 
  }
...
#' @param objectName - objectname from which quotes to be read. This is generally from data.makeQuotesFromGDrive.
...
#' @export
data.getOneMinQuotes <- function(objectName,
                     stock     = NULL,
                     fut = NULL,
                     trim = "T23:55") {
  if (is.xts(objectName)){
    stop("In data.getOneMinQuotes : objectNAme is an xts. Only data frame is needed.")
  }
